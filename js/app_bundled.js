...
(function () {
  ...
  function processAllStats(mediaList = [], subjectsList = [], eventsList = [], weights = {}) {
    ...
    // Calculate First Met date per pair of subjects across all events
  ...
  function renderEventDetailsPage(stats) {
    ...
    // 🫂 Subject Combinations Calculation
    let combosHTML = '';
    ...
    // 🗓️ Nearby Events Timeline Calculation (±7 Days)
    ...
    // Clicking participant card navigates to Subject Details
    ...
    // Clicking 💦 count badge opens prompt to edit 💦 count
  ...
  function setupEventListeners() {
    ...
    document.getElementById('saveSubjectBtn')?.addEventListener('click', async () => {
      const nameInput = document.getElementById('subjectNameInput');
      const genderSelect = document.getElementById('subjectGenderSelect');
      const val = nameInput ? nameInput.value.trim() : '';
      const genderVal = genderSelect ? genderSelect.value : 'Female';
      if (!val) { alert('Please enter a subject name.'); return; }
      const pId = await db.getActiveProfileId();
      const newSub = { id: 'sub-' + Date.now(), profileId: pId, name: val, gender: genderVal, groupId: 'green', avatarUrl: null };
      await db.put('subjects', newSub);
      document.getElementById('addSubjectModal').classList.remove('active');
      await loadAppState();
      renderCurrentView();
  ...
  async function regenerateAllThumbnails() {
    ...
    document.getElementById('saveWizardBtn')?.addEventListener('click', async () => {
      ...
      const whereData = {
        countryOnline: document.getElementById('wizardWhereCountryOnline')?.value.trim() || '',
        stateApp: document.getElementById('wizardWhereStateApp')?.value.trim() || '',
        city: document.getElementById('wizardWhereCity')?.value.trim() || '',
        place: document.getElementById('wizardWherePlace')?.value.trim() || '',
        zone: document.getElementById('wizardWhereZone')?.value.trim() || '',
        gcode: document.getElementById('wizardWhereGCode')?.value.trim() || ''
      };
