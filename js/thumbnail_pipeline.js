...
async function createCompressedThumbnailWithRotation(fileType, dataUrl, rotateAngle = 0) {
  return new Promise((resolve) => {
    const canvas = document.createElement('canvas');
    canvas.width = 200;
    canvas.height = 200;
    ...
    const drawRotatedCrop = (source) => {
      ...
      ctx.rotate(((((rotateAngle % 360) + 360) % 360) * Math.PI) / 180);
      ctx.drawImage(source, sx, sy, cropSize, cropSize, -100, -100, 200, 200);
      ctx.restore();
    };
...
window.createCompressedThumbnailWithRotation = createCompressedThumbnailWithRotation;
