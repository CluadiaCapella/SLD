/**
 * Section 1: Database Manager (IndexedDB), Schema, Action Tags & Group Mappers
 */
...
function getGroupDisplayTitle(g) {
  if (!g) return '⚪ Unassigned / No Subject Group';
  let cleanName = g.name.trim();
  cleanName = cleanName.replace(/^[🟤🟣🟢🟡🟠🔴🔵⚪⚫⭐\s]+/g, '').trim();
  return `${g.emoji || '⚪'} ${cleanName}`;
}

window.DB_NAME = DB_NAME;
window.DB_VERSION = DB_VERSION;
window.ACTION_TAG_NAMES = ACTION_TAG_NAMES;
window.DEFAULT_ACTION_POINTS = DEFAULT_ACTION_POINTS;
window.SUB_ACTION_TAGS = SUB_ACTION_TAGS;
...
window.getSubjectGroup = getSubjectGroup;
window.getGroupDisplayTitle = getGroupDisplayTitle;
window.getSubjectGroupOptionsHTML = getSubjectGroupOptionsHTML;
window.handleGroupSelectAction = handleGroupSelectAction;
...
window.getMediaHighestPriorityGroupBorderClass = getMediaHighestPriorityGroupBorderClass;
window.calculateMediaStarRating = calculateMediaStarRating;
window.getStarRatingLabel = getStarRatingLabel;
